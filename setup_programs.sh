...
# .desktop 파일을 바탕화면으로 복사
echo "[diagnosis] SLAMNAV2.desktop과 diagnositc.desktop 파일을 바탕화면으로 복사합니다."
cp diagnosis/SLAMNAV2.desktop "$DESKTOP_DIR/"
cp diagnosis/diagnositc.desktop "$DESKTOP_DIR/"

# 스크립트 파일을 홈으로 복사
echo "[diagnosis] diagnositc.sh와 slamnav2.sh 파일을 홈 디렉토리로 복사합니다."
cp diagnosis/diagnositc.sh "$HOME/"
cp diagnosis/slamnav2.sh "$HOME/"

# .desktop 파일 잠금 해제 (아이콘 잠금 해제)
...
if [ ! -d "slamnav2" ]; then
    ...
    git clone https://github.com/rainbow-mobile/slamnav2.git
else
    echo "[slamnav2] 리포지토리가 이미 존재합니다. 최신 상태로 업데이트합니다."
    cd slamnav2 && git pull && cd ..
fi

cd slamnav2

echo "[slamnav2] 원격 브랜치 목록:"
# 원격 브랜치 목록을 가져와서 번호로 출력
