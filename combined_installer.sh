...
declare -A SCRIPTS
SCRIPTS[1]="install_udev_rules.sh"
SCRIPTS[2]="set_teamviewer.sh"
SCRIPTS[3]="setup_env_path.sh"
SCRIPTS[4]="setup_fms2.sh"
SCRIPTS[5]="setup_programs_slamanv_shortcut.sh"
SCRIPTS[6]="setup_sensor2.sh"
SCRIPTS[7]="setup_srv_h4-ultra.sh"
SCRIPTS[8]="setup_system_build_env_s100-2.sh"

print_menu() {
  echo "설치할 스크립트 번호를 선택하세요:"
  echo "  1) install_udev_rules.sh"
  echo "  2) set_teamviewer.sh"
  echo "  3) setup_env_path.sh"
  echo "  4) setup_fms2.sh"
  echo "  5) setup_programs_slamanv_shortcut.sh"
  echo "  6) setup_sensor2.sh"
  echo "  7) setup_srv_h4-ultra.sh"
  echo "  8) setup_system_build_env_s100-2.sh"
  echo "  a) 모두 설치"
}
