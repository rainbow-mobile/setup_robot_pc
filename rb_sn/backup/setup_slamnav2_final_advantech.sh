...
###############################################################################
## GRUB-efi 문제 방지를 위한 패키지 제거 및 잠금
###############################################################################
log "[초기화] grub-efi-amd64-signed 제거 및 재설치 방지"

if dpkg -s grub-efi-amd64-signed &>/dev/null; then
  apt-get purge -y grub-efi-amd64-signed || true
