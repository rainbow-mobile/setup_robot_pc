...
#──────────────────────────────────────────────────────────────────────────────
## 🗺️ /home/maps 폴더가 없으면 생성
#──────────────────────────────────────────────────────────────────────────────
#MAP_DIR="/home/maps"
#if [ ! -d "$MAP_DIR" ]; then
#  echo "[INFO] $MAP_DIR 디렉터리가 존재하지 않아 새로 생성합니다."
#  mkdir -p "$MAP_DIR"
#  chown "$REAL_USER:$REAL_USER" "$MAP_DIR"
#fi


#──────────────────────────────────────────────────────────────────────────────
## 💾 /data/maps 폴더 생성 및 권한 부여
#──────────────────────────────────────────────────────────────────────────────
DATA_MAPS_DIR="/data/maps"
if [ ! -d "$DATA_MAPS_DIR" ]; then
  log "[INFO] $DATA_MAPS_DIR 디렉터리가 없어 새로 생성합니다."
  mkdir -p "$DATA_MAPS_DIR"
fi

log "[INFO] /data 디렉터리 및 하위 모든 파일/폴더에 777 권한을 부여합니다."
chmod -R 777 /data
...
run_2() {  # setup_sensor2.sh
  ...
  if [ ! -d "$INSTALL_BASE/OrbbecSDK" ]; then
      log "[CLONE] OrbbecSDK"
      if ! as_user "git clone https://github.com/SimonLee9/OrbbecSDK.git \"$INSTALL_BASE/OrbbecSDK\""; then
          log "[ERROR] OrbbecSDK 클론 실패"
          return 1
      fi
      if ! as_user "cd \"$INSTALL_BASE/OrbbecSDK\" && git checkout 2L"; then
      ...
      # 올바른 버전인지 확인
      if ! as_user "cd \"$INSTALL_BASE/OrbbecSDK\" && git checkout 2L 2>/dev/null"; then
          log "[WARN] OrbbecSDK 브랜치 체크아웃 실패, 현재 버전 유지"
      fi
      # 헤더 파일 확인
